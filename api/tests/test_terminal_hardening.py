...
def test_terminal_mega_ls_blocked_absolute_path(monkeypatch):
    ...
    response = client.post("/api/terminal", json={"command": "mega-ls /etc"}, headers={"Origin": "http://testserver"})
    assert response.status_code == 200
    data = response.json()
    assert data["ok"] is False
    assert "Blocked: local path access outside /data" in data["output"]
...
def test_terminal_mega_ls_allowed_remote_path(monkeypatch):
    ...
    response = client.post(
        "/api/terminal", json={"command": "mega-ls /Root/MyFiles"}, headers={"Origin": "http://testserver"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_terminal_blocked_relative_path_traversal(monkeypatch):
    ...
    response = client.post(
        "/api/terminal", json={"command": "mega-ls ../../../etc/passwd"}, headers={"Origin": "http://testserver"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["ok"] is False
