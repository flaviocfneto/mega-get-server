...
def _extract_wget2_urls(parts: list[str]) -> list[str]:
    flags_with_args = {
        "-O", "--output-document", "-o", "--output-file", "-i", "--input-file",
        "-B", "--base", "-D", "--domains", "-I", "--include-directories",
        "-X", "--exclude-directories", "-C", "--config", "-P", "--directory-prefix",
        "-U", "--user-agent", "-t", "--tries", "-T", "--timeout", "--limit-rate",
        "--user", "--password", "--proxy-user", "--proxy-password", "--quota",
    }
    urls = []
    skip_next = False
