...
Use these steps to find why the web UI does not work while `mega-get` works in the container.
...
(or `netstat -tlnp` if `ss` is not available)
...
If 8080 is not listed, the Flet app did not bind (crashed or wrong port).
...
- **000** or connection error - nothing is listening on 8080, or curl is not in the image (then try from the host, step 5).
...
(Use the host/port you actually use: e.g. `http://192.168.1.x:8383` if you are on another machine.)
...
- A request with type **websocket** (or "WS") - status **101 Switching Protocols**. If it is red/failed: mixed content, or proxy/firewall blocking WebSockets.
...
If this works but the Gluetun setup does not, the issue is port publishing (8383:8080 on protonvpn) when using `network_mode: service:protonvpn`.
